...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent) {
    ...
    setFixedSize(340, 540);

    // ── Equation label (shows running expression above the main display) ────
    m_equation = new QLabel("");
    m_equation->setAlignment(Qt::AlignRight);
    m_equation->setFont(QFont("Arial", 13));
    m_equation->setFixedHeight(28);
    m_equation->setStyleSheet("color:#888888; background:#1c1c1e; "
                              "padding-right:12px;");
    ...
    // ── Stack: equation label + display + grid ─────────────────────────────
    auto* vbox = new QVBoxLayout;
    vbox->setContentsMargins(12, 12, 12, 12);
    vbox->setSpacing(0);
    vbox->addWidget(m_equation);
    ...
    // Map display symbol → internal op character
...
void MainWindow::onDigitClicked(const QString& digit) {
    if (m_newNumber) {
        m_display->setText(digit == "." ? "0." : digit);
        m_newNumber = false;
    } else {
    ...
    // Keep equation label current while user types the second operand
    if (!m_pendingOp.isEmpty())
        m_equation->setText(m_operand1Str + " " + m_pendingOp + " " + m_display->text());
}

void MainWindow::onOperatorClicked(const QString& op) {
    m_operand1    = m_display->text().toDouble();
    m_operand1Str = m_display->text();
    m_pendingOp   = op;
    m_newNumber   = true;
    // Show "123 +" in the equation line
    m_equation->setText(m_operand1Str + " " + m_pendingOp);
}
...
void MainWindow::onEquals() {
    ...
    double result   = 0.0;
    const QString op2Str = m_display->text();
    ...
    // Show full equation: "123 + 3 ="
    m_equation->setText(m_operand1Str + " " + m_pendingOp + " " + op2Str + " =");
    m_display->setText(resultStr);
    m_pendingOp.clear();
    m_newNumber = true;
}
...
void MainWindow::onBackspace() {
    ...
    if (!m_pendingOp.isEmpty())
        m_equation->setText(m_operand1Str + " " + m_pendingOp + " " + text);
}
